...
async function updateUBlockCounter() {
    ...
    const blockedTrackersCount = await promisify(r => chrome.runtime.sendMessage(ublock, {
...
async function updateFingerprintsCounter() {
    ...
    const blockedFingerprintsCount = await promisify(r => chrome.extension.sendMessage(epicfsprotection, {
...
function userAgentVersionNumber() {
    ...
    a.open("GET", "http://updates.epicbrowser.com/extensions/newtab/useragent.xml", true); // Release Server
    ...
    a.onreadystatechange = function() {
        ...
        if (4 == a.readyState) {
            ...
            var newVersion = c.getElementsByTagName("version")[0].childNodes[0].nodeValue; // Version number from the userAgent.xml
            ...
            if (update == true) {
                ...
                console.log("AlokShilad "+ uAgentVersion)
            }
        }
    };
            ...
            chrome.storage.local.get(['slots'], function (data) {
                ...
                if (Array.isArray(slots)) {
                    ...
                    for (let i = 0; i < slots.length; i++) {
                        ...
                        if (slot.url && (slot.url.startsWith('http://') || slot.url.startsWith('https://'))) {
                            userSites[`site${i}`] = {
                                title: slot.title || '',
                                url: slot.url
                            };
